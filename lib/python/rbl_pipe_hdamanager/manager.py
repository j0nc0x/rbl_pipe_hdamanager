...
from rbl_pipe_hdamanager import repo
...
class HDAManager(object):
    ...
    def __init__(
        ...
        """Initialise the HDAManager class."""
        logger.info("Initialising HDA Manager")
        self.hda_repos = dict()
        self.releases = list()
    ...
    def get_repo_paths(self):
        """
        Get a list of HDA repository paths from the environment.

        Returns:
            (list): A list of HDA repositories in the current environment.
        """
        repo_paths = list()
        rebellion_hdas = os.getenv("REBELLION_HDAS")
        if rebellion_hdas:
            repo_paths = rebellion_hdas.split(os.pathsep)
    ...
    def initialise_repositories(self):
        ...
        # Loop over all repositories by looking at REBELLION_HDAS env var
        repo_paths = self.get_repo_paths()
        for path in repo_paths:
            # Create the repository object
            hda_repo = repo.HDARepo(self, path)
            name = hda_repo.get_name()

            # Add to repositories list
            self.hda_repos[name] = hda_repo

    def load_all(self):
        """Load all nodeTypes."""
        for repo_name in self.hda_repos:
            self.hda_repos.get(repo_name).load()
